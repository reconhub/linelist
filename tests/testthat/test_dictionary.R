...
test_that("check_dictionary() will thwart attempts to import incorrect epivars", {
  expect_error(set_dictionary(pi),
               "is not a data frame but a numeric")
  expect_error(set_dictionary("non/existant/file.txt"), 
               "The file 'non/existant/file.txt' does not appear to exist")
  expect_error(set_dictionary(iris), 
               "x does not have 3 columns but 5")
  expect_error(set_dictionary(iris[1:3]), 
